...
select distinct(industry)
-- select unique industry			-- Doesn't work
from movies;
...
select min(imdb_rating) min_rating, max(imdb_rating) max_rating, round(avg(imdb_rating), 1) as avg_rating
from movies;
...
-- select movie_id, title, industry, ifnull(imdb_rating, 0), studio
select movie_id, title, industry, coalesce(imdb_rating, 0), studio
from movies;

-- 33. Give me the average rating of Bollywood movies
select round(avg(imdb_rating), 1) as avg_bollywood_rating
from movies
where industry = "Bollywood";

-- 34 .Print the average rating of Hollywood movies
select round(avg(imdb_rating), 1) as avg_hollywood_rating
from movies
where industry = "Hollywood";

-- 35. Print the average imdb_rating of all the industries rounded off to one decimal places
